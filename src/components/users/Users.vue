			...
			<el-table :data="userList" style="width: 100%">
				...
				<el-table-column align="center" label="操作">
					<template slot-scope="scope">
						...
						<el-tooltip effect="dark" content="分配角色" placement="top" transition="in">
							<el-button type="success" icon="el-icon-check" circle @click="setRole(scope.row)"></el-button>
						</el-tooltip>
...
<script>
    export default {
        ...
        methods: {
            ...
            async setUserRole() {
				if (!this.selectedRoleName) return this.$message.error("请选择角色")
                const {data:res} = await this.$http.put(`users/${this.userInfo.id}/role`,{rid:this.selectedRoleName})
				if (res.meta.status !== 200) return this.$message.error(res.meta.msg)
